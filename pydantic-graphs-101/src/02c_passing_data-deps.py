...
@dataclass
class FirstNode(BaseNode[GraphState, GraphDeps, str]):
    async def run(self, ctx: GraphRunContext[GraphState, GraphDeps]) -> End[str]:
        ...
        system_message = SystemMessage(
            ...
            "Always use the user's name in the response: " + ctx.deps.userid
        )
...
@dataclass
class SecondNode(BaseNode[GraphState, GraphDeps, str]):
